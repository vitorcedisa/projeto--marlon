...
def lambda_handler(event, context):
    """
    Função Lambda para marcar um pedido como recebido pelo cliente.
    ...
    """
    logger.info("Evento recebido para marcar como recebido: %s", json.dumps(event))
    ...
    try:
        ...
        logger.info(f"Pedido [{order.id}] marcado como recebido pelo cliente")
        
        return success_response({
            'mensagem': 'Pedido marcado como recebido',
            'pedido': order.to_dict()
        })
        
    except Exception as e:
        logger.error("Erro na execução da Lambda received: %s", str(e))
